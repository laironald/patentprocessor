...
def domestic_block_remove_sql():
    ...
    stmt = """SELECT  11,
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  blk_split(SEPARATE_ROW(a.City, %d)) = b.blkcity
           AND  a.state = b.state
           AND  a.country = 'US'
...
def domestic_first3_jaro_winkler_sql():
    ...
    stmt = """SELECT  (10+jarow(blk_split(SEPARATE_ROW(a.City, %d)),
                b.BlkCity)) AS Jaro,
                a.cnt as cnt,
                a.city as CityA,
                a.state as StateA,
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  SUBSTR(blk_split(SEPARATE_ROW(a.City, %d)),1,3) = b.City3
           AND  a.state = b.state
           AND  a.country = 'US'
         WHERE  jaro > %s
...
def domestic_last4_jaro_winkler_sql():
    ...
    stmt = """SELECT  (10+jarow(blk_split(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  rev_wrd(blk_split(SEPARATE_ROW(a.City, %d)),4) = b.City4R
           AND  a.state = b.state
           AND  a.country = 'US'
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
...
def foreign_block_split_sql():
    ...
    stmt = """SELECT  21,
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  blk_split(SEPARATE_ROW(a.City, %d)) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  separator_count(a.City) >= %d
           AND  a.City != "" """
    return stmt;
...
def foreign_first3_jaro_winkler_sql():
    ...
    stmt = """SELECT  (20+jarow(blk_split(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  SUBSTR(blk_split(SEPARATE_ROW(a.City, %d)),1,3) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
           AND  a.City != ""
...
def foreign_last4_jaro_winkler_sql():
    ...
    stmt = """SELECT  (20+jarow(blk_split(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  rev_wrd(blk_split(SEPARATE_ROW(a.City, %d)),4) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
           AND  a.City != ""
