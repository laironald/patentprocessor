...
def domestic_sql():
   ...
   INNER JOIN  usloc AS b
           ON  SEPARATE_ROW(CityA, %d) = b.city
          AND  StateA = b.state
          AND  CountryA = 'US'
...
def domestic_block_remove_sql():
    ...
    stmt = """SELECT  11,
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  remove_spaces(SEPARATE_ROW(a.City, %d)) = b.blkcity
           AND  a.state = b.state
           AND  a.country = 'US'
...
def domestic_first3_jaro_winkler_sql():
    ...
    stmt = """SELECT  (10+jarow(remove_spaces(SEPARATE_ROW(a.City, %d)),
                b.BlkCity)) AS Jaro,
                a.cnt as cnt,
                a.city as CityA,
                a.state as StateA,
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  SUBSTR(remove_spaces(SEPARATE_ROW(a.City, %d)),1,3) = b.City3
           AND  a.state = b.state
           AND  a.country = 'US'
         WHERE  jaro > %s
...
def domestic_last4_jaro_winkler_sql():
    ...
    stmt = """SELECT  (10+jarow(remove_spaces(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  usloc AS b
            ON  rev_wrd(remove_spaces(SEPARATE_ROW(a.City, %d)),4) = b.City4R
           AND  a.state = b.state
           AND  a.country = 'US'
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
...
def foreign_full_name_1_sql():
    ...
    INNER JOIN  loctbl.gnsloc AS b
            ON  SEPARATE_ROW(a.City, %d) = b.full_name_ro
           AND  a.country = b.cc1
         WHERE  separator_count(a.City) >= %d
           AND  a.City!="" """
    return stmt;
...
def foreign_full_name_2_sql():
    ...
    stmt = """SELECT  21,
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  SEPARATE_ROW(a.City, %d) = b.full_name_nd_ro
           AND  a.country = b.cc1
         WHERE  separator_count(a.City) >= %d
           AND  a.City != "" """
    return stmt;
...
def foreign_short_form_sql():
    ...
    stmt = """SELECT  21,
          ...
          FROM  loc           AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  SEPARATE_ROW(a.City, %d) = b.short_form
           AND  a.country = b.cc1
         WHERE  separator_count(a.City) >= %d
           AND  a.City != "" """
    return stmt;
...
def foreign_block_split_sql():
    ...
    stmt = """SELECT  21,
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  remove_spaces(SEPARATE_ROW(a.City, %d)) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  separator_count(a.City) >= %d
           AND  a.City != "" """
    return stmt;
...
def foreign_first3_jaro_winkler_sql():
    ...
    stmt = """SELECT  (20+jarow(remove_spaces(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  SUBSTR(remove_spaces(SEPARATE_ROW(a.City, %d)),1,3) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
           AND  a.City != ""
...
def foreign_last4_jaro_winkler_sql():
    ...
    stmt = """SELECT  (20+jarow(remove_spaces(SEPARATE_ROW(a.City, %d)),
          ...
          FROM  loc AS a
    INNER JOIN  loctbl.gnsloc AS b
            ON  rev_wrd(remove_spaces(SEPARATE_ROW(a.City, %d)),4) = b.sort_name_ro
           AND  a.country = b.cc1
         WHERE  jaro > %s
           AND  separator_count(a.City) >= %d
           AND  a.City != ""
...
def domestic_zipcode_sql():
    ...
    stmt = """SELECT  31,
                ...
                b.longitude
          FROM  (SELECT  *,
                         (SEPARATE_ROW(zipcode,0)+0) as Zip2
                   FROM  loc
                  WHERE  Zipcode != ''
                    AND  Country = 'US') AS a
    INNER JOIN  usloc AS b
...
####            ON  SEPARATE_ROW(a.City, %d)=b.city AND a.country=b.state
